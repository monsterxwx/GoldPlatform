  ...
  <div class="min-h-screen bg-[#f2f2f6] pb-10 font-sans text-[#1c1c1e]">
    ...
    <header class=" pb-6 px-6 bg-white shadow-sm rounded-b-3xl">
      <div class="flex justify-between items-center mb-2">
        <h1 class="text-2xl font-bold tracking-tight">实时金价</h1>
        <button 
          @click="handleRefresh" 
          class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center active:bg-gray-200 transition-colors"
        >
          <svg :class="['w-4 h-4 text-gray-600', isRefreshing ? 'animate-spin' : '']" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>
        </button>
      </div>
    ...
    <main class="px-4 mt-6 space-y-4">
      ...
      <section class="bg-white rounded-3xl p-6 shadow-sm">
        ...
        <!-- Profit -->
        <div class="mb-6">
          <div class="text-sm text-gray-500 mb-1">总计收益 (历史+当前)</div>
          <div :class="[
            'text-3xl font-bold',
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-[#f2f2f6] rounded-2xl p-4">
            <div class="text-xs text-gray-500 mb-1">当前持仓收益</div>
            <div :class="[
              'text-lg font-bold',
          ...
          <div class="bg-[#f2f2f6] rounded-2xl p-4">
            <div class="text-xs text-gray-500 mb-1">历史累计收益</div>
            <div :class="[
              'text-lg font-bold',
        ...
        <div class="mt-6 pt-6 border-t border-gray-100 grid grid-cols-3 gap-2 text-center">
          <div>
            <div class="text-xs text-gray-400 mb-1">持仓均价</div>
            <div class="text-sm font-semibold">{{ goldStore.averagePrice.toFixed(2) }}</div>
          </div>
          <div>
            <div class="text-xs text-gray-400 mb-1">持仓克数</div>
            <div class="text-sm font-semibold">{{ goldStore.totalGrams.toFixed(2) }}</div>
          </div>
          <div>
            <div class="text-xs text-gray-400 mb-1">持仓总额</div>
            <div class="text-sm font-semibold">{{ goldStore.totalAmount.toFixed(2) }}</div>
          </div>
        </div>
      </section>
      ...
      <section class="bg-white rounded-3xl p-6 shadow-sm flex items-center justify-between">
        <div>
          <h2 class="text-sm text-gray-500 mb-1">回本保本金价</h2>
          <div class="text-xl font-bold">{{ goldStore.breakEvenPrice > 0 ? goldStore.breakEvenPrice.toFixed(2) : '--' }}</div>
        </div>
        <div class="text-right">
          <div class="text-xs text-gray-400 mb-1">距当前金价差额</div>
          ...
          <div class="text-sm font-semibold text-gray-300" v-else>--</div>
        </div>
      </section>

      <!-- Simulator / Add Position -->
      <section class="bg-white rounded-3xl p-6 shadow-sm">
        <h2 class="text-lg font-semibold mb-4">交易计算与操作</h2>
        ...
        <div class="mt-4 p-4 bg-blue-50 rounded-2xl" v-if="tradeType === 'buy'">
          <div class="text-xs text-blue-600 mb-1">模拟加仓后均价将变为:</div>
        ...
        <div class="mt-4 p-4 bg-orange-50 rounded-2xl" v-if="tradeType === 'sell'">
          <div class="text-xs text-orange-600 mb-1">模拟减仓将实现收益:</div>
    ...
    <div v-if="showInitModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity">
      <div class="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
        <h2 class="text-xl font-bold text-center mb-6">初始配置</h2>
        <div class="space-y-4">
          <div>
        ...
        <div class="mt-8 flex gap-3">
          <button @click="showInitModal = false" class="flex-1 py-3.5 bg-gray-100 text-gray-700 font-semibold rounded-xl">取消</button>
          <button @click="handleSaveInit" class="flex-1 py-3.5 bg-[#007aff] text-white font-semibold rounded-xl shadow-sm">保存</button>
        </div>
      </div>
    </div>
