<template>
  <div class="h-full flex flex-col bg-[#f2f2f6] font-sans">
    ...
    <header class="pt-4 pb-3 px-4 bg-white/80 backdrop-blur-md border-b border-gray-100 shrink-0 flex justify-between items-center z-10 sticky top-0">
      <div class="text-lg font-semibold tracking-tight text-gray-800">
        智能分析
      </div>
    ...
    <div v-if="showConfig" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity">
      <div class="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input
              ...
              class="w-full bg-[#f2f2f6] rounded-xl px-4 py-3 outline-none border-none ring-0 text-sm"
            ...
            <div class="relative">
              ...
              <div class="absolute right-0 top-0 bottom-0 w-10 flex items-center justify-center text-gray-400 cursor-pointer" @mousedown.prevent="showModelList = !showModelList">
                <svg
                  ...
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  :class="['transition-transform', showModelList ? 'rotate-180' : '']"
        ...
        <div class="mt-8 flex gap-3">
          <button @click="showConfig = false" class="flex-1 py-3.5 bg-gray-100 text-gray-700 font-semibold rounded-xl">
            取消
          </button>
          <button @click="saveConfig" class="flex-1 py-3.5 bg-[#007aff] text-white font-semibold rounded-xl shadow-sm">
            保存
          </button>
        </div>
      </div>
    ...
    <main class="flex-1 overflow-y-auto p-4 space-y-4" ref="chatContainer">
      ...
      <div v-if="isTyping" class="flex w-full justify-start">
        ...
        <div class="bg-white rounded-2xl rounded-tl-sm shadow-sm px-4 py-3 flex items-center space-x-1">
          ...
          <div class="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce" style="animation-delay: 0.15s" />
          <div class="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce" style="animation-delay: 0.3s" />
        </div>
      </div>
    </main>
...
.markdown-body pre { overflow-x: auto; margin: 0.5em 0; padding: 12px; border-radius: 8px; background: #f6f8fa; }
.markdown-body code { padding: 2px 4px; font-size: 0.9em; font-family: monospace; border-radius: 4px; background: #f6f8fa; }
.markdown-body ul { margin-bottom: 0.5em; padding-left: 20px; list-style-type: disc; }
.markdown-body ol { margin-bottom: 0.5em; padding-left: 20px; list-style-type: decimal; }
</style>
